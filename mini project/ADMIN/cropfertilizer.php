...
<body>
	...
	<div class="container-fluid" style="padding:0px;margin-top:51px;">
		...
		<div class="col-md-10" style="padding:20px 20px; border-left: 1px solid #ccc;">
			<div class="col-md-12">
				<p> CROP FERTILIZER
					<button type="button" class="btn btn-primary btn-sm pull-right" data-toggle="modal" data-target="#addfert">
				...
				<div class="table-responsive">
					<table class="table table-bordered">
						...
						<?php
							$query = "SELECT  b.id as id, a.name as cname, c.name as fname from crop a join crop_fert b on a.crop_id = b.crop_id join fertilizer c on c.fert_id = c.fert_id";
							...
							if ($count > 0) {
								...
								<tr>
									...
									<td>
										<a href="javascript:void(0);" class="deletebtn" data-column="id" data-table="crop_fert" data-id="<?php echo $row["id"]?>">
		...
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">Add Crop Fertilizer</h4>
				</div>
				<div class="modal-body">
					<div class="form-group">
	                    ...
	                    <label class="control-label col-lg-2">Fertilizer</label>
	                    <div class="col-lg-10">
	                    	<select name="fertilizer" id="fertilizer" class="form-control" >
								...
								// output data of each row
								while($row = $result->fetch_assoc()){ ?>
		                        <option value="<?php echo $row['fert_id'];?>" data-catId="<?php echo $row['fert_id'];?>"><?php echo $row["name"];?></option>
								<?php }?>
		                    </select>
	                    </div>
	                </div>
	...
	<script>
		$(document).ready(function(){
			...
			$('#addcrops').click(function(){
				var val1 = $("#crop").val();
				var val2 = $("#fertilizer").val();
				$.ajax({
					type : 'POST',
					url  : 'operation.php',
					data : {'functionname':'add_combo', 'id1':'crop_id', 'id2':'fert_id', 'val1':val1, 'val2':val2, 'table':'crop_fert'},
				}).done(function(data) {
					if(data == 1){
						//TODO Alert here to show success message
						window.location.href = 'cropfertilizer.php';
					}
				}).fail(function(error) {
					console.log(error);
				});
